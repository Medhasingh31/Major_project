...
def load_graph_from_graphml(graphml_path):
    """Load the GraphML file and normalize node/edge representations."""
    if not os.path.exists(graphml_path):
        raise FileNotFoundError(f"GraphML file not found: {graphml_path}")
        
    G = nx.read_graphml(graphml_path)
    
    # Ensure node coordinate attributes are floats/ints and clean IDs
    cleaned_G = nx.Graph()
    cleaned_G.graph.update(G.graph)
    
    # Map old nodes to cleaned node structures
    node_mapping = {}
...
def calculate_route_alternatives(
    ...
    """
    Parses the GraphML network, snaps the points, penalizes overlap with reference network,
    discovers alternative paths with parallel corridor penalization, filters candidates to satisfy
    minimum diversity requirements, and scores each route option.
    """
    ...
    # 2. Connect start and end pins
    start_node, start_snapped = connect_point_to_graph(G, start_pt, "start")
    end_node, end_snapped = connect_point_to_graph(G, end_pt, "end")
    
    # 3. Calculate edge costs based on baseline overlap
    for u, v, data in G.edges(data=True):
        ...
        # Penalize if overlapping (baseline avoidance weight)
        if not allow_existing_roads and overlap_ratio > 0.1:
            weight = 1e9  # Set high cost to block edge
        else:
    ...
    # 4. Generate alternative paths using Dijkstra weight penalization
    temp_G = G.copy()
